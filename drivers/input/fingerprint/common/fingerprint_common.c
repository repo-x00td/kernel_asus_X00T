...
static const struct of_device_id fp_common_table[] = {
...
int commonfp_power_on(void)
{
	...
	pr_info("%s: power on OK!", __func__);
	return 0;
}

int commonfp_power_off(void)
{
	pr_info("%s: power off OK!", __func__);
	return 0;
}
 ...
 * so if use it,the value is right
 */
#if USE_COMMON_PINCTRL
...
static int fp_common_probe(struct platform_device *pdev)
{
	...
	if (!fp_g.fp_pinctrl) {
		pr_err("%s: failed to get commonfp pinctrl\n", __func__);
		return -1;
	}
	...
	if (ret) {
		...
		pr_info("%s: select commonfp_irq_active stateOK!\n", __func__);
	}
	...
	if (ret) {
		pr_err("%s: failed to request pm660lGPIO(5). err=%d\n",
		       __func__, ret);
		return -1;
	}
...
static int fp_common_remove(struct platform_device *pdev)
{
	...
	dev_info(dev, "%s release all gpio suscess!", __func__);
#endif
...
static int __init fingerprint_resource_init(void)
{
	...
	if (ret) {
		pr_err("%s: register commonfp platform driver failed:%d\n",
		       __func__, ret);
		return ret;
	}
	pr_info("%s: commonfp init OK!\n", __func__);
