...
static void smblib_uusb_removal(struct smb_charger *chg)
{
	...
	if (gpio_is_valid(global_gpio->ADC_SW_EN)) {
		...
		if (val == 1) {
			...
			if (rc)
				...
				pr_err("%s Pull low USBSW_S\n", __func__);
		} else
			pr_info("%s: get USBSW_S gpio val %d\n", __func__, val);
	}
	...
	if (val == 1) {
		...
		if (rc)
			...
			pr_info("%s: Pull low ADC_VH_EN\n", __func__);
	} else
		pr_info("%s: get ADC_VH_EN gpio val %d\n", __func__, val);
				  ...
				  const union power_supply_propval *val)
{
	...
	pr_info("%s : smartchg_stop_flag = %d\n", __func__, smartchg_stop_flag);
	power_supply_changed(chg->batt_psy);
	return rc;
}
...
void asus_batt_RTC_work(struct work_struct *dat)
{
	...
	pr_info("%s: alarm start after %ds\n", __func__, RTCSetInterval);
...
int smbchg_jeita_judge_state(int old_State, int batt_tempr)
{
	...
	pr_info("%s: result_State=%d\n", __func__, result_State);
	return result_State;
}
...
static int jeita_status_regs_write(u8 chg_en, u8 FV_CFG, u8 FCC)
{
	...
	if (rc < 0) {
		pr_err("%s: Couldn't write charging_enable rc = %d\n",
		       __func__, rc);
		return rc;
	}
	...
	if (rc < 0) {
		pr_err("%s: Couldn't write FV_CFG_reg_value rc = %d\n",
		       __func__, rc);
		return rc;
	}
	...
	if (rc < 0) {
		pr_err("%s: Couldn't write FCC_reg_value rc = %d\n",
		       __func__, rc);
		return rc;
	}
...
void asus_update_usb_connector_state(struct smb_charger *chip)
{
	...
	if (IS_ERR(chip->gpio12_vadc_dev)) {
		pr_err("%s: error get chg_alert vadc rc=%d\n", __func__, rc);
		rc = PTR_ERR(chip->gpio12_vadc_dev);
		if (rc != -EPROBE_DEFER)
			pr_err("%s: Couldn't get chg_alert vadc rc = %d\n",
			       __func__, rc);
		return;
	}
	...
	if (chip->gpio12_vadc_dev) {
		...
		pr_info("%s: qpnp_vadc_read : phy_volta = %lld\n",
			__func__, phy_volta);
	} else {
		pr_err("%s: NONE gpio12_vadc_dev\n", __func__);
		return;
	}
	...
	if (phy_volta < CHG_ALERT_HOT_NTC_VOLTAFE) {
		...
		if (rc < 0)
			dev_err(chip->dev, "Couldn't set CMD_OTG_REG rc=%d\n",
				rc);
		pr_info("%s:USB connector hot, suspend charger and otg\n",
			__func__);

	} else if ((phy_volta >= CHG_ALERT_HOT_NTC_VOLTAFE) &&
		   (phy_volta <= CHG_ALERT_WARM_NTC_VOLTAGE)) {
		if (usb_alert_usb_otg_disable == true)
			pr_err("%s:USB alert former state is hot, now is warm\n",
			       __func__);
		else
			pr_info("%s:USB alert former state is GOOD, now is warm",
				__func__);
		...
		if (rc < 0)
			dev_err(chip->dev, "Couldn't set CMD_OTG_REG rc=%d\n",
				rc);
		pr_info("%s:USB connector temp is GOOD, recover charge\n",
			__func__);
	}
}
...
void jeita_rule(void)
{
	...
	if (bat_temp >= START_REPORT_BAT_TEMPRATURE) {
		...
		pr_info("[%s]line=%d: bat_temp=%d\n",
			__func__, __LINE__, bat_temp);
	}
...
void asus_min_monitor_work(struct work_struct *work)
{
	...
	if (charger_limit_enable_flag) {
		if (asus_get_prop_batt_capacity(smbchg_dev) >=
		    charger_limit_value) {
			pr_info("%s: charger limit is enable & over, stop charging\n",
				__func__);
		...
		pr_info("%s: charger limit flag = %d, limit-soc = %d & over, stop charging\n",
...
void asus_chg_flow_work(struct work_struct *work)
{
	...
	case FLOAT_CHARGER_BIT:
		...
		if (rc < 0)
			pr_err("%s: Couldn't read fast_CURRENT_LIMIT_CFG_REG\n",
			       __func__);
		pr_info("%s: usbmode_USBIN_1_cc=0x%x\n", __func__, USBIN_1_cc);
	...
	case CDP_CHARGER_BIT:
		...
		if (rc < 0)
			pr_err("%s: Couldn't read fast_CURRENT_LIMIT_CFG_REG\n",
			       __func__);
		pr_info("%s: cdp_USBIN_1_cc=0x%x\n", __func__, USBIN_1_cc);

		asus_smblib_rerun_aicl(smbchg_dev);
		smblib_asus_monitor_start(smbchg_dev, 0);
		break;
	case OCP_CHARGER_BIT:
		pr_info("%s: OCP_CHARGER_BIT", __func__);
...
static void CHG_TYPE_judge(struct smb_charger *chg)
{
	...
	pr_info("%s: ASUS_ADAPTER_ID=%d\n", __func__, ASUS_ADAPTER_ID);
}
...
void asus_adapter_adc_work(struct work_struct *work)
{
	...
	pr_info("enter1 %s USBIN_CURRENT_LIMIT_reg=0x%x\n",
		__func__, USBIN_CURRENT_LIMIT_reg);
	rc = smblib_write(smbchg_dev, USBIN_CURRENT_LIMIT_CFG_REG, 0x01);
	rc = smblib_read(smbchg_dev, USBIN_CURRENT_LIMIT_CFG_REG,
			 &USBIN_CURRENT_LIMIT_reg);
	if (rc < 0)
		pr_err("%s: Failed to set USBIN_OPTIONS_1_CFG_REG\n", __func__);
	pr_info("enter2 %s USBIN_CURRENT_LIMIT_reg=0x%x\n",
		__func__, USBIN_CURRENT_LIMIT_reg);
	...
	if (rc)
		...
		pr_info("%s: Pull low USBSW_S\n", __func__);

	pr_info("%s: ASUS_ADAPTER_ID = %s, setting mA = 0x%x\n",
